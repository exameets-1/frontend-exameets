...
export const AddAiGovtJob = ({ isOpen, onClose, onSubmit }) => {
    const [govtJobDetails, setGovtJobDetails] = useState('');
    ...
    const handleSubmit = async (e) => {
        ...
        if (!govtJobDetails.trim()) {
            setError('Please enter government job details');
            return;
        }
        ...
        try {
            await onSubmit(govtJobDetails.trim());
            setGovtJobDetails('');
            setError('');
        } catch (err) {
            setError(err.message || 'Failed to create AI govt job');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleInputChange = (e) => {
        setGovtJobDetails(e.target.value);
        if (error) {
            setError('');
        }
    };
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="space-y-4">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-green-700 dark:text-green-400">
                                        Enter govt job details in any format. AI will automatically extract key fields like department, location, category, deadlines, eligibility, salary, and application link.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <label 
                                htmlFor="govtJobDetails" 
                                className="block text-sm font-bold text-gray-700 dark:text-gray-200"
                            >
                                Government Job Details <span className="text-red-500">*</span>
                            </label>

                            <textarea
                                id="govtJobDetails"
                                name="govtJobDetails"
                                value={govtJobDetails}
                                onChange={handleInputChange}
                                placeholder="Example: 
The Indian Railways is hiring Junior Engineers. Application closes August 31, 2025. Location: All India. Eligibility: Diploma in Engineering. Pay Scale: Level 6. Apply at: https://rrb.gov.in"
                                rows={12}
                            ...
                            <div className="flex justify-between text-xs text-gray-500 dark:text-gray-400">
                                ...
                                <span>{govtJobDetails.length} characters</span>
                            </div>
                        </div>
                    ...
                    <div className="pt-6 border-t border-gray-200 dark:border-gray-700">
                        <div className="flex justify-end gap-4">
                            ...
                            <button
                                type="submit"
                                disabled={isLoading || !govtJobDetails.trim() || govtJobDetails.trim().length < 10}
