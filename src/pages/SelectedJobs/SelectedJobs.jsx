...
export default function SelectedJobs() {
  const { matchedJobs, loading: userLoading, user } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    // Only fetch if user and preferences are available
  ...
  }, [dispatch, user?.preferences?.notifications_about]);

  const isLoading = userLoading || admissionsLoading || internshipsLoading || scholarshipsLoading || resultsLoading;
  ...
  if (!user || Object.keys(user).length === 0) {
    return (
      <div className="h-full bg-gray-100 dark:bg-gray-800 p-5">
        <div className="h-full max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 flex justify-center items-center">
          <div className="w-10 h-10 border-4 border-[#015990] border-t-transparent rounded-full animate-spin"></div>
        </div>
      </div>
    );
    ...
    <div className="h-full bg-gray-100 dark:bg-gray-800 p-5">
      <div className="h-full max-w-6xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-5 flex flex-col">
        <div className="mb-6">
          ...
          <h2 className="text-xl text-gray-600 dark:text-gray-300 text-center mb-6">
            Preference Selected: {user?.preferences?.notifications_about || 'Not Set'}
          </h2>
        </div>
